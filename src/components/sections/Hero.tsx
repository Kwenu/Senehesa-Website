...
import { ShieldCheckIcon, HeartHandshakeIcon, MapPinnedIcon } from 'lucide-react';
...
export function Hero() {
  return (
    <section className="relative overflow-hidden bg-cloud">
      <div className="pointer-events-none absolute -right-24 -top-24 h-96 w-96 rounded-full bg-mist" />
      <div className="relative mx-auto grid w-full max-w-7xl items-center gap-12 px-5 py-14 sm:px-8 sm:py-20 lg:grid-cols-2 lg:gap-16 lg:py-24">
        <motion.div
          ...
          <p className="mt-6 max-w-xl text-lg leading-relaxed text-ink/80 sm:text-xl">
            Reliable elder care, assisted transportation and daily support for your loved
            ones across Sri Lanka — helping you stay connected, even when you can't be
            there.
          </p>
          ...
          <ul className="mt-6 flex flex-wrap gap-x-6 gap-y-3">
            {badges.map((badge) =>
            <li key={badge.label} className="flex items-center gap-2 text-ink/75">
        ...
        <motion.div
          ...
          <div className="absolute -bottom-6 left-4 right-4 rounded-3xl border border-navy/10 bg-white p-5 shadow-lift sm:left-8 sm:right-auto sm:max-w-xs">
            <p className="font-display text-lg font-bold text-navy">Door-to-door care</p>
